...
const apiCalls = function(input) {
  ...
  return axios.get(`https://maps.googleapis.com/maps/api/place/details/json?location=49.2827,79.1207&radius=2500&keyword=${userInputSerialized}&type=cafe&key=${googleApiKey}`)
    .then(res => {
      if((res.data.status !== "ZERO_RESULTS") && (res.data.results[0].types.includes('cafe') || res.data.results[0].name === `${userInput}`)) {
        inputType = "food";
        pass = true;
      }
    ...
    .then(res => {
      ...
      if((res.data.status !== "ZERO_RESULTS") && (res.data.results[0].types.includes('restaurant') || res.data.results[0].name === `${userInput}`)) {
          inputType = "food";
          pass = true;
      }
