...
const apiCalls = function(input) {
  ...
  const categories = ['film', 'food', 'book', 'product', 'other'];
  const classifier = function(input) {
    const value = input.trim().split(" ");
    if (value[-1].includes(categories)) {
      if(value[-1].toLowerCase() === 'film') {
        return 'film';
      } else if (value[-1].toLowerCase() === 'food') {
        return 'food';
      } else if (value[-1].toLowerCase() === 'book'){
        return 'book';
      } else if (value[-1].toLowerCase() === 'product') {
        return 'product';
      } else if (value[-1].toLowerCase() === 'other') {
        return 'other';
      }
    }
  }
